...
func TestAccSaviyntDynamicAttributeResource(t *testing.T) {
	filePath := "dynamic_attribute_resource_test_data.json"
	createCfg := util.LoadConnectorData(t, filePath, "create")
	updateCfg := util.LoadConnectorData(t, filePath, "update")
	resourceName := "saviynt_dynamic_attribute_resource.da"
...
func testAccDynamicAttributeResourceConfig(operation string) string {
	jsonPath := "/Users/nandini.choukimath/Desktop/terraform-provider-saviynt/internal/provider/dynamic_attribute_resource_test_data.json"
	return fmt.Sprintf(`
provider "saviynt" {
  server_url = "%s"
  username   = "%s"
